...
class Trainer_builder:
    def __init__(self, train_file_name):
        ...
        self.trainer = {'sentences_count': 0, 'transition': {}}
        self.read()
    ...
    def read(self):
        with open(self.train_file_name, 'r') as file:
            for line in file.readlines():
                ...
                self.build_trainer_char(line)

    def build_trainer_char(self,line):
        cleaned_string=re.findall(r'\w|[(),.]|[-!?\'"]|[ ]', line)
        line_length=len(cleaned_string)
        TRAIN_LETTERS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789(),.-!?\"' "
        #print cleaned_string
        for index, character in enumerate(cleaned_string):
            #character=cleaned_string[index]
            #print "char=" + character
            #print "next=" + cleaned_string[index - 1]
            #print index

            if self.trainer['transition'].get(character, False) == False:
                self.trainer['transition'][character]={t: 0 for t in TRAIN_LETTERS}
                self.trainer['transition'][character]["_last"]=0
                self.trainer['transition'][character]["_first"] = 0
                if index==0:
                    self.trainer['transition'][character]["_first"] += 1
                self.trainer['transition'][character]['_count']=1
                if index < line_length - 1:
                    self.trainer['transition'][character][cleaned_string[index+1]]=1
            else:

                #print self.trainer['transition'][character]
                if index == 0:
                    self.trainer['transition'][character]["_first"] += 1
                self.trainer['transition'][character]['_count'] += 1
                if index < line_length-1:
                    ...
                    self.trainer['transition'][character]['_last']+=1

        for letter in TRAIN_LETTERS:
            if self.trainer['transition'].get(letter, False) == False:
                self.trainer['transition'][letter] = {t: 0 for t in TRAIN_LETTERS}
                self.trainer['transition'][letter]["_count"]=0
                self.trainer['transition'][letter]["_first"] = 0
                self.trainer['transition'][letter]["_last"] = 0
